...
describe("test get methods /videos router", () => {
  ...
  it("should return one item in list with success", async () => {
    ...
    expect(actual.result.body).toEqual([createdEntity]);
  });

  it("should return two items in list with success", async () => {
    const data = GetCreateVideoInputModel({});
    const { resultEntities: emptyResultEntities } = await getEntities({});
    expect(emptyResultEntities).toEqual([]);

    const { createdEntity: ce1 } = await createEntity({ data });
    const { createdEntity: ce2 } = await createEntity({ data });

    const { resultEntities } = await getEntities({});

    expect(resultEntities).toEqual([ce1, ce2]);
  });
  ...
  it("should return 56 items in list with success", async () => {
    ...
    expect(actual.result.body).toEqual(createdEntities);
  });
  ...
  it("should return NOT_FOUND in case if video doesn't exist", async () => {
    const actual = await getEntity({
      data: 999999,
      expectedStatus: HTTP_CODES.HTTP_STATUS_NOT_FOUND,
    });

    expect(actual.result.status).toBe(HTTP_CODES.HTTP_STATUS_NOT_FOUND);
  });
